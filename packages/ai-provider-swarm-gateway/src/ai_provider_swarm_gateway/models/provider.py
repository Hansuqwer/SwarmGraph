...
Capability = Literal[
    "chat", "vision", "embeddings", "audio", "image", "video", "rerank", "tools", "code", "unknown"
]
...
class ProviderQuota(BaseModel):
    ...
    def _unknown_limits_are_not_free(self) -> ProviderQuota:
        ...
        """
        if self.confidence == "unknown" and (self.free_daily_usage or self.free_monthly_usage):
            if "UNVERIFIED" not in str(self.free_daily_usage or "") and "UNVERIFIED" not in str(
                self.free_monthly_usage or ""
            ):
                self.notes.append(
...
class ProviderInfo(BaseModel):
    ...
    def is_api_free(self) -> bool:
        ...
        return bool(
            ...
            and (
                self.quota.free_daily_usage
                or self.quota.free_monthly_usage
                or self.quota.trial_credits
            )
        )
