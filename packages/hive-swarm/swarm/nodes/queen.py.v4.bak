"""Queen node — patched (v4 — forwards llm_settings into shared_context).

History:
  F-15A: loud RuntimeError if Send is unavailable but expected
  F-15B: real _adaptive_decompose escalating to mesh on weak prior agreement
  F-15-CORR4: dead `import secrets` removed
  F-25B: mesh prompts diversified (each worker gets distinct task description)
  F-13C: imports QUEEN_NODE_NAMES from models.types (single source of truth)
  F-27A: queen forwards retrieved_context into QueenDirective.shared_context
  v4 (this revision): queen also forwards llm_settings (backend / provider /
     max_tokens / temperature / timeout / role_provider_overrides /
     include_retrieved_patterns / include_objective) so workers route
...
def _hierarchical_decompose(
    objective: str,
    max_agents: int,
    *,
    prior_agreement: float = 1.0,
...
def _mesh_decompose(
    ...
    *,
    ...
    """F-25B: each worker gets a distinct perspective prompt."""
...
def _adaptive_decompose(
    ...
    *,
    ...
    """F-15B: escalate to mesh when prior consensus was weak (< 0.5)."""
...
# ── llm_settings extraction (v4) ─────────────────────────────────────────

def _llm_settings_from_config(config: Any) -> dict[str, Any]:
    """Pull optional llm_* fields off SwarmConfig. Defensive: missing fields
    silently default — matches stub-mode behaviour for unmodified configs."""
    return {
        ...
        "include_retrieved_patterns": getattr(
            config, "llm_include_retrieved_patterns", True
        ),
...
def queen_node(state: dict[str, Any]) -> list[Any]:
    """Decompose objective + Send fan-out."""
    ...
    # F-15B: prior agreement → adaptive escalation
    prior_agreement = (
    ...
    sub_tasks = decompose(
        swarm.objective,
        swarm.config.max_agents,
        prior_agreement=prior_agreement,
    )

    # F-15-LG3: enforce agent cap defensively
    ...
    # F-27A: include retrieved patterns in shared_context
    retrieved_patterns = list(swarm.retrieved_context) if swarm.retrieved_context else []

    # v4: forward llm_settings so workers can pick a backend
    ...
    for idx, (desc, role) in enumerate(sub_tasks):
        ...
        directive = QueenDirective(
            ...
            shared_context={
                ...
                "retrieved_patterns": retrieved_patterns,    # F-27A
                "llm_settings": llm_settings,                # v4
            },
        )
    ...
    # F-15A: loud failure if Send unavailable but expected
...
def medium_agent_node(state: dict[str, Any]) -> dict[str, Any]:
    """Tier 2: single LLM call (still stub for now; v4 leaves this for a
    future patch since tier-2 doesn't go through the queen→worker path)."""
    swarm = SwarmState.model_validate(state)
    swarm.status = "executing"
