"""SwarmState + SwarmCheckpoint — patched (v7).

History:
  v4–v6 lineage preserved.
  F-09A: assert_no_drift raises before mutating
  F-09B: schema_version
  F-19A: approval_consumed guard
  F-27A: retrieved_context
  v6: 3-mode anti-drift (off / keyword / embedding)

v7 — F-18-CORR2: When `anti_drift_similarity_threshold == 0`, the
     `_check_drift_embedding` cosine check `cosine ≥ 0.0` is technically
     true for orthogonal vectors but is a meaningless pass. Coalesce
     threshold=0 into mode="off" semantics so users get the obvious
     behaviour: "set threshold to 0 if you want to skip drift detection".
"""
...
class SwarmState(HardenedModel):
    ...
    # ── Anti-drift (v7: F-18-CORR2 threshold=0 coalesce) ─────────────────
    ...
    def check_drift(
        ...
        *,
        ...
        v7 — F-18-CORR2: threshold=0 is treated as mode='off' regardless
        of the configured mode. Otherwise threshold=0 + mode='embedding'
        would always pass via cosine ≥ 0 (technically true for orthogonal
        vectors, semantically meaningless). Threshold=0 + mode='keyword'
        also always passes (overlap ≥ 0). Coalescing makes the intent
        explicit and consistent across modes.
        """
        if not self.config.anti_drift_enabled:
            return True

        # F-18-CORR2: zero threshold = effectively no drift detection
    ...
    def _check_drift_embedding(
        ...
        """Cosine similarity in embedding space (F-18-CORR2: never reached
        when threshold=0 because check_drift short-circuits earlier)."""
