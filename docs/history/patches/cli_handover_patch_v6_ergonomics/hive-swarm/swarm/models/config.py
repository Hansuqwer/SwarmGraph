"""SwarmConfig — patched (v6).

v6 NEW fields:
  - anti_drift_mode: "off" | "keyword" | "embedding"  (default: "keyword" — v5 behaviour)
  - cost_tracking_enabled: bool                        (default: True; populates WorkerResult.usage.cost_usd)
  - llm_stream_enabled: bool                           (default: False; opt-in streaming)

History preserved:
  F-10A, F-10-T1, F-10-DOC1, v4 llm_* fields, v5 llm_role_model_overrides.

Backwards compatible: every new field defaults preserve pre-v6 behaviour.
"""
...
class SwarmConfig(FrozenModel):
    ...
    # ── Anti-drift (v6: 3-mode) ───────────────────────────────────────────
    anti_drift_enabled: bool = True
    anti_drift_mode: AntiDriftMode = Field(
        default="keyword",
        description=(
            "Drift detection strategy. "
            "'off' = skip drift detection entirely (recommended for code-gen workloads). "
            "'keyword' = v5 keyword-overlap heuristic (default for back-compat). "
            "'embedding' = cosine similarity via injected EmbeddingProvider; "
            "if no embedder bound, falls back to keyword."
        ),
    )
    ...
    # ── v6: streaming + cost tracking ─────────────────────────────────────
    llm_stream_enabled: bool = Field(
        default=False,
        description=(
            "When True, workers use dispatcher.dispatch_stream() and concatenate "
            "chunks. Useful for progress visibility on long-running swarms."
        ),
    )
    cost_tracking_enabled: bool = Field(
        default=True,
        description=(
            "When True, WorkerResult.usage.cost_usd is populated via "
            "swarm_shared.pricing.estimate_cost. Lookup misses (unknown model id) "
            "leave cost_usd=None — never raise."
        ),
    )
...
__all__ = ["SwarmConfig", "LLMBackend", "AntiDriftMode"]
