"""9Router adapter — OpenAI-compatible local router (v6: adds chat_stream).

v6 additions:
  - chat_stream(messages, ...) → Iterator[StreamChunk-shaped dict]
    Real SSE parser for the data: ... \\n events. Reuses the same data:
    sentinel logic the response parser already handles.

v3 history:
  - HTTP via stdlib urllib (zero deps)
  - data: [DONE] sentinel handling on non-streaming responses
  - 5 method aliases (chat / chat_completion / complete / call / invoke)
  - 5 env-var aliases for the API key
  - call() → upstream GatewayResponse (per your local fix)
"""
...
DEFAULT_STREAM_TIMEOUT_SECONDS = 300.0   # streams may take longer
...
class _HttpClient:
    ...
    def post_json_stream(
        ...
        *,
        ...
        """Yield raw SSE lines (incl. 'data:' prefix). Caller parses."""
        body = json.dumps(payload).encode("utf-8")
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {api_key}",
...
def _parse_sse_data_line(line: str) -> dict[str, Any] | None:
    """Parse a single 'data: {...}' SSE line. Returns None for [DONE] or non-data."""
...
def _stream_event_to_chunk(event: dict[str, Any]) -> dict[str, Any]:
    """OpenAI-shape SSE event → {delta, finish_reason}."""
    ...
    if isinstance(delta_obj, dict):
        ...
        # Some routes stream reasoning — treat it as content if no content present
        if not delta_text:
...
class NineRouterAdapter(_BaseProviderAdapter):
    ...
    def chat_stream(
        ...
        *,
        ...
        """Yield {"delta": str, "finish_reason": str} for each SSE event.

        Adds `"stream": true` to the payload. Parses `data: {...}` SSE lines,
        ignoring `[DONE]` sentinel and empty pings.
        """
        api_key = _resolve_api_key(self._explicit_api_key)
        if not api_key:
            raise PermissionError(
    ...
    # call() returning upstream GatewayResponse — preserved per local fix
    # (left to local edits; the v6 patch does NOT replace your call())
