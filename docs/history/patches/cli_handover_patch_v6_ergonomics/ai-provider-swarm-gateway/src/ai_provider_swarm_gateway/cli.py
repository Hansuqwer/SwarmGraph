"""ai-provider-swarm-gateway — Typer CLI (v6).

v6 additions (additive on v5):
  - `swarm --stream` flag: prints chunked progress via dispatch_stream
  - `quota show --since N[smhd]` filter
  - Cost rollup in `swarm` JSON output (total_cost_usd)
  - Two local CLI fixes baked in (canonical):
      * Empty quota message: "no usage recorded yet"
      * Missing-gateway error: "Vendor them into..." actionable line

Preserves v5 surface:
  version, quota show/increment/reset/set-reset, providers list (filters),
  inspect-state, route, swarm.
"""
...
def _parse_duration_to_seconds(s: str) -> int:
    """Parse '30s' / '5m' / '1h' / '7d' → seconds. Raises ValueError on bad input."""
...
def quota_show(
    ...
    since: Optional[str] = typer.Option(
        None, "--since",
        help="Filter to entries with reset_at within this window of NOW. "
             "Format: 30s | 5m | 1h | 7d. Empty = no filter.",
    ),
    ...
    if provider:
        ...
        if not all_usage:
            # Baked CLI fix #1: lower-case message preserved
    ...
    if cutoff is not None:
        ...
        for pid, win, u in rows:
            ra = u.reset_at
            if ra is None:
                # Conservative: include entries with no reset_at (unbounded windows)
        ...
        rows = filtered
        if not rows:
            _print(f"[no usage in last {since}]")
            raise typer.Exit(0)
    ...
    if _HAS_RICH and _console:
        title = f"Quota usage ({tracker.storage_path})"
        if cutoff:
            title += f" — since {since}"
        t = Table(title=title)
...
# ── route — preserved + baked CLI fix #2 ────────────────────────────────

# Baked CLI fix #2: explicit "Vendor them into..." actionable error
...
# ── swarm — v6 (adds --stream, cost rollup) ──────────────────────────────
...
def swarm(
    ...
    # v6
    anti_drift_mode: str = typer.Option(
        "keyword", "--anti-drift",
        help="off | keyword | embedding (v6: 'off' avoids the iteration storm)",
    ),
    stream: bool = typer.Option(
        False, "--stream",
        help="Stream worker output chunk-by-chunk (requires backend=gateway).",
    ),
    no_cost: bool = typer.Option(
        False, "--no-cost",
        help="Disable cost computation (default: on).",
    ),
) -> None:
    ...
    if json_output:
        ...
        if _HAS_RICH and _console:
            ...
            cost_str = (
                f"${total_cost:.4f}" if cost_known else "—"
            )
            _console.print(
