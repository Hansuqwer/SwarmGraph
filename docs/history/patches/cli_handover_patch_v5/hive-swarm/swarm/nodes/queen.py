"""Queen + tier-1/tier-2 nodes — patched (v5 — medium_agent_node uses gateway).

History:
  F-15A, F-15B, F-15-CORR4, F-25B, F-13C, F-27A, v4 llm_settings forwarding,
  F-15-LG4 (your local fix: queen Send fan-out as conditional edge in factory.py)
v5: medium_agent_node now routes through the gateway dispatcher with role
    "coder" (single-agent generalist). Falls back to stub if backend=stub.
    Tier-2 stays a single LLM call — no swarm spawn — but it's a real call.
"""
...
# ── Decomposition strategies (unchanged from v4) ──────────────────────────
...
# ── Queen node (unchanged from v4 forwarding semantics) ──────────────────
...
def fast_agent_node(state: dict[str, Any]) -> dict[str, Any]:
    """Tier 1: deterministic transform, no LLM. Unchanged from v4."""
    swarm = SwarmState.model_validate(state)
...
# ── Tier 2: single LLM call (v5: through gateway) ────────────────────────

def medium_agent_node(state: dict[str, Any]) -> dict[str, Any]:
    """Tier 2: single LLM call, no swarm spawn.

    v5: routes through the same dispatcher as workers. Uses role "coder" as
        a generalist persona. When `llm_backend="stub"`, behaves identically
        to v4 (returns the deterministic placeholder string).

    Failures stay typed: any WorkerLLMError → swarm.fail(). The graph then
    routes through judge_node which detects the empty/failed state.
    """
    swarm = SwarmState.model_validate(state)
    swarm.status = "executing"

    # Build the same shared_context shape queen uses for tier-3 workers
    shared_context = {
        "iteration": swarm.iteration,
        "objective": swarm.objective,
        "retrieved_patterns": list(swarm.retrieved_context) if swarm.retrieved_context else [],
    ...
    try:
        ...
        if backend == "stub":
            # Preserve the v4 stub label for tier-2 specifically
