"""ai-provider-swarm-gateway — Typer CLI (v5 — adds `swarm` subcommand).

v5 adds:
  ai-provider-gateway swarm --prompt "..." [--topology ...] [--json]
    Routes a prompt through hive-swarm with the gateway under it.
    Stub mode by default; --backend gateway flips to real LLM dispatch.

Preserves v4 surface:
  version, quota show/increment/reset/set-reset, providers list (with filters),
  inspect-state, route (lifted from stub).
"""
...
def quota_reset(
    ...
    tracker = QuotaTracker(storage_path=_resolve_storage_path(storage))
    # Use existing reset_usage if your local tracker has it; else fall back to
    # the set_reset_time-now path that v3 used.
...
def _try_load_registry() -> Optional[list[dict]]:
    """Load registry, supporting upstream {providers: [...]} + legacy bare list."""
...
def route(
    ...
    try:
        ...
        _err(f"❌ Upstream gateway modules missing: {e}")
        raise typer.Exit(2)
...
def inspect_state(json_output: bool = typer.Option(False, "--json")) -> None:
    """Print the GatewayState field schema."""
    try:
        from .models.state import GatewayState  # type: ignore
    except ImportError as e:
...
@app.command("swarm")
def swarm(
    prompt: str = typer.Option(..., "--prompt", "-p", help="The objective for the swarm."),
    topology: str = typer.Option(
        "hierarchical", "--topology",
        help="hierarchical | mesh | ring | star | adaptive",
    ),
    consensus: str = typer.Option("raft", "--consensus", help="raft | bft | gossip | majority"),
    max_agents: int = typer.Option(5, "--max-agents", min=1, max=100),
    backend: str = typer.Option(
        "stub", "--backend",
        help="stub (deterministic) | gateway (real LLM via providers).",
    ),
    provider: str = typer.Option(
        "9router", "--provider",
        help="Default provider id when --backend=gateway.",
    ),
    model: Optional[str] = typer.Option(
        None, "--model",
        help="Default model id (empty → adapter default).",
    ),
    max_tokens: int = typer.Option(512, "--max-tokens", min=1),
    temperature: float = typer.Option(0.0, "--temperature", min=0.0, max=2.0),
    sona: bool = typer.Option(True, "--sona/--no-sona", help="Enable SONA memory loop."),
    auto_approve: bool = typer.Option(
        True, "--auto-approve/--no-auto-approve",
        help="Raise the HITL threshold so the run completes non-interactively.",
    ),
    thread_id: Optional[str] = typer.Option(None, "--thread-id"),
    json_output: bool = typer.Option(False, "--json"),
    show_workers: bool = typer.Option(False, "--show-workers", help="Print per-worker outputs."),
) -> None:
    """Route a prompt through hive-swarm with the gateway underneath.

    Closes the operator surface: a single command runs the full
    Pydantic v2 + LangGraph + Swarm + Gateway stack end-to-end.

    Examples:
        ai-provider-gateway swarm --prompt "Implement auth refresh"
        ai-provider-gateway swarm --prompt "..." --backend gateway --provider 9router
        ai-provider-gateway swarm --prompt "..." --topology mesh --consensus gossip --json
    """
    try:
    ...
    # Build the SwarmConfig with gateway-friendly defaults
    try:
        ...
        if auto_approve:
            config_kwargs["require_approval_above_risk"] = 0.99   # effectively never
    ...
    if json_output:
        ...
        if _HAS_RICH and _console:
            ...
            _console.print(Panel(prompt, title="Objective"))
            _console.print(
                f"[dim]status[/dim] {final.status}  "
                f"[dim]iterations[/dim] {final.iteration}  "
                f"[dim]sona[/dim] {final.sona_cycle_count}  "
                f"[dim]workers[/dim] {len(final.worker_results)}"
            )
            _console.print(
                f"[dim]tokens[/dim] in={total_in} out={total_out}  "
                f"[dim]backend[/dim] {backend} ({provider})"
            )
            if final.final_output:
                _console.print(Panel(final.final_output, title="Final output"))
            if show_workers:
                t = Table(title="Workers")
                t.add_column("Role"); t.add_column("OK"); t.add_column("Tokens (in/out)")
            ...
            print(f"final output:    {final.final_output[:300]}")
