...
History:
  ...
  v6 — F-13A-CORR1: replace `operator.add` reducer with dedupe-merge.
  v7.1 — F-13-CORR2: register ALL 5 queen aliases as nodes (was: only one).
         The conditional edge `route_task` returns any of the 5 queen
         names; LangGraph's compiler rejects the routing dict if a
         destination name isn't a registered node. Topology dispatch
         still happens INSIDE queen_node (via _DECOMPOSE_FN), so all 5
         aliases point at the same function.
  v7.1 — F-13-CORR3: mock graph mirrors real graph SONA edges. Tier 1
         and Tier 2 paths now call distill_node, matching the real
         graph's `add_edge("fast_agent", "distill_node")` etc.
"""
...
# ── F-13A-CORR1: dedupe-merge reducer ────────────────────────────────────
...
def _merge_worker_results(
    ...
    Key: (agent_id, task_id). Right wins on collision.

    F-13A-CORR1: replaces operator.add (which caused 80-vs-5 inflation
    when LangGraph re-emitted state during retry/replay).
    """
    if not left:
...
def build_swarm_graph(
    ...
    F-13-CORR2: registers ALL 5 queen aliases as nodes pointing at
    queen_node, so route_task can return any of them without LangGraph's
    compiler rejecting the routing dict. Topology selection happens
    INSIDE queen_node via _DECOMPOSE_FN[swarm.config.topology], not at
    the graph-edge level.
    """
    ...
    # F-13-CORR2: register ALL queen aliases (not just config.topology's)
    ...
    for name in all_queen_names:
        builder.add_edge(name, "collect_results")
...
# ── Mock graph (F-13-CORR3: SONA on tier 1/2) ────────────────────────────

class _MockCompiledGraph:
    """LangGraph-compatible stub for offline testing.

    F-13-CORR3: tier-1 and tier-2 paths now call distill_node, matching
    the real graph's `add_edge("fast_agent", "distill_node")` etc.
    Without this, mock-mode tests would skip SONA pattern storage and
    silently lie about working.
    """
    ...
    def invoke(self, state, config=None):
        ...
        if tier == "tier1_fast":
            ...
            # F-13-CORR3: mirror real graph add_edge("fast_agent", "distill_node")
            state = distill_node(state)
        elif tier == "tier2_medium":
            state = medium_agent_node(state)
            # F-13-CORR3: mirror real graph add_edge("medium_agent", "distill_node")
            state = distill_node(state)
        else:
